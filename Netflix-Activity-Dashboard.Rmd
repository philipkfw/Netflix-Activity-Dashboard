...
manipulated_df <- df %>% 
  ...
  # filter(Title != "The Naked Director")
...
#   inputId = "v_DateRange",
#   label = "Date Range",
#   start = min(manipulated_df$Date),
#   end = max(manipulated_df$Date),
...
#     arrange(Viewer, Type, desc(`Watch Time (Minutes)`)) %>% 
#     ungroup() %>% 
...
renderPlotly({
  manipulated_df %>% 
    ...
    filter(Rank <= 5) %>% 
    # filter(Viewer == "Metis") %>% 
    ggplot(aes(x = reorder(Title, `Watch Time (Minutes)`), y = `Watch Time (Minutes)`)) +
    geom_bar(stat = 'identity', fill = alpha(metis_color, 0.85)) +
    coord_flip() +
    theme_bw() +
    labs(x = "", y = "") +
    facet_grid(~Viewer) +
...
# top10titles_philip <- renderPlotly({
#   manipulated_df %>%
#     filter(str_detect(User, if_else(input$v_User == "All", "", input$v_User))) %>%
#     filter(str_detect(Type, if_else(input$v_Type == "All", "", input$v_Type))) %>%
#     filter(str_detect(Genre, if_else(input$v_Genre == "All", "", input$v_Genre))) %>%
...
#     mutate(Rank = row_number()) %>%
#     filter(Rank <= 5) %>%
#     filter(Viewer == "Philip") %>%
#     ggplot(aes(x = reorder(Title, `Watch Time (Minutes)`), y = `Watch Time (Minutes)`)) +
#     geom_bar(stat = 'identity', fill = alpha(philip_color, 0.9)) +
#     coord_flip() +
#     theme_bw() +
#     labs(x = "", y = "") +
#     scale_y_continuous(expand = expansion(mult = c(0, .1))) # reduces the gap between y axis
...
#     filter(Viewer == "Both") %>% 
#     ggplot(aes(x = reorder(Title, `Watch Time (Minutes)`), y = `Watch Time (Minutes)`)) +
#     geom_bar(stat = 'identity', fill = alpha(both_color, 0.85)) +
#     coord_flip() +
#     theme_bw() +
#     labs(x = "", y = "") +
#     scale_y_continuous(expand = expansion(mult = c(0, .1))) # reduces the gap between y axis
# })
# 
# subplot(top10titles_metis, top10titles_philip, top10titles_both, nrows = 3)
